...
<svg class='decor' viewBox='0 0 400 700' preserveAspectRatio='xMidYMid slice'
  ...
  <g stroke='rgba(255,255,255,.35)' stroke-width='1'>
    ...
    <line x1='343' y1='170' x2='343' y2='255'/><line x1='349' y1='170' x2='349' y2='255'/>
  </g>
...
# ⭐ FIX #1: Helper — HTML string ko BINA indentation ke render karo
...
""".replace("__IC_USER__", IC_USER).replace("__IC_MAIL__", IC_MAIL).replace("__IC_LOCK__", IC_LOCK))
...
if ss.screen == "welcome":
    render_html("""
    <style>
    .block-container{max-width:430px!important;margin:0 auto;padding:0 0 2rem!important;
    background:transparent!important;box-shadow:none!important;}
    .splash{position:relative;height:92vh;min-height:620px;overflow:hidden;
    ...
    # ⭐ FIX #2: href ki jagah button (Cloud multipage ?page= safe)
    if st.button("Get Started", key="btn_start"):
        ss.screen = "signup"
        st.rerun()
    st.markdown("""<style>
    #btn_start + div, div[data-testid="stButton"]:has(#btn_start) {}
    div[data-testid="stButton"] > button[kind="secondary"] {}
    </style>""", unsafe_allow_html=True)
    st.stop()

# ═════════════════════════════════════════════
#  AUTH LAYOUT (signup / login)
# ═════════════════════════════════════════════
render_html("""
<style>
...
.switch-line{text-align:center;color:#6b7280;font-size:14.5px;padding:16px 0 2px;}
[data-testid="stAlert"]{border-radius:14px!important;}
</style>
""")

# Get Started button ko pill style dena (welcome screen ke liye override)
if ss.screen == "welcome":
    pass
...
if ss.screen == "signup":
    render_html("""
    <div class="hero">__DECOR__<div class="logo-md">HMF</div></div>
